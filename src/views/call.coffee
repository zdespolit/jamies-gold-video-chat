...
class CallView extends Marionette.LayoutView 
	...
	handleLocalStreamStart: (stream) =>
		if @audio_
			@attachAudio(stream, 'local')
		else
			attachmediastream(stream, @$('div.video-local video')[0],
				autoplay: true
				mirror: true
				muted: true
			)
	...
	handleRemoteVideoStart: (peer) =>
		if @audio_
			@attachAudio(peer.stream, 'remote', peer.id)
	...
	attachAudio: (stream, type, peerId) =>
		if type == 'remote'			
			player = new Audio()
			attachMediaStream(stream, player)
			player.play()
